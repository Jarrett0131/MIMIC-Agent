...
// Load agent-server/.env. __dirname only exists in CJS (tsx dev/test runs);
// the Mastra bundle is ESM, where the dev server loads .env itself, so the
// cwd fallback is enough there.
dotenv.config({
  path:
...
// ---------------------------------------------------------------------------
// LLM prompt scenarios (merged from config/llmScenarios.ts)
// ---------------------------------------------------------------------------
