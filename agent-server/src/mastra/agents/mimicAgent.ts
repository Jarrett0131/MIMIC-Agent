...
import { getPatientTool } from "../tools/getPatient";
import { getDiagnosesTool } from "../tools/getDiagnoses";
import { getLabsTool } from "../tools/getLabs";
import { getVitalsTool } from "../tools/getVitals";
 ...
 * Unified MIMIC agent.
 *
 * This replaces the legacy regex classifier (services/classifier.ts) + static
 * router (services/router.ts): instead of pattern-matching the question to one
 * fixed tool, the LLM decides which tool(s) to call via tool-calling. All five
 * tools wrap the existing agent-server logic, so the data boundary is unchanged.
 */
