...
def _get_imported_vitals(hadm_id: int, keyword: str) -> list[VitalRecordData] | None:
    ...
    return [
        record
        for record in imported_patient.vitals
        if isinstance(record.get("label"), str)
        and keyword_lower in record["label"].strip().lower()
    ]


def _get_merged_vitals(hadm_id: int, keyword: str) -> pd.DataFrame:
    if not keyword.strip():
        return pd.DataFrame()

    chartevents = data_loader.chartevents
    items = data_loader.d_items
    if chartevents.empty or items.empty:
        return pd.DataFrame()
    if "hadm_id" not in chartevents.columns or "itemid" not in chartevents.columns:
        return pd.DataFrame()
    if "itemid" not in items.columns or "label" not in items.columns:
        return pd.DataFrame()

    merged = chartevents.merge(
        items[["itemid", "label"]].drop_duplicates(subset=["itemid"]),
        on="itemid",
        how="left",
    )
...
def _to_vital_records(dataframe: pd.DataFrame) -> list[VitalRecordData]:
    records: list[VitalRecordData] = []
    for _, row in dataframe.iterrows():
        records.append(
            {
                "subject_id": _int_or_none(row.get("subject_id")),
                "hadm_id": _int_or_none(row.get("hadm_id")),
                "stay_id": _int_or_none(row.get("stay_id")),
                ...
                "warning": _int_or_none(row.get("warning")),
            }
        )
...
def get_vitals_by_keyword(
    ...
    keyword: str,
    limit: int = 20,
) -> list[VitalRecordData]:
    imported_records = _get_imported_vitals(hadm_id, keyword)
    if imported_records is not None:
        return imported_records[:limit]

    merged = _get_merged_vitals(hadm_id, keyword)
    if merged.empty:
        return []

    return _to_vital_records(merged.head(limit))
